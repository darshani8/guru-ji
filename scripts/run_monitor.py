...
enabled, each picking up where the last stopped. Alerts go to each
...
async def _run(runtime, institution_id: str | None, map_mode: bool = False) -> list[dict[str, object]]:
...
async def _loop(runtime, institution_id: str | None, every: float, map_mode: bool) -> None:
    while True:
        try:
            print(json.dumps(await _run(runtime, institution_id, map_mode), indent=2, default=str), flush=True)
        except SystemExit:
            raise
...
def main() -> None:
    ...
    try:
        if args.every:
            asyncio.run(_loop(runtime, args.institution, args.every, args.map))
        else:
            print(json.dumps(asyncio.run(_run(runtime, args.institution, args.map)), indent=2, default=str))
    finally:
        runtime.close()
