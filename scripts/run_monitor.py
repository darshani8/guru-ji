...
since the last digest) once its last one is 23 hours old; run it hourly
(``--digest --every 3600``) so a failed digest is retried within the hour
and a restart never sends one twice. Alerts go to each
    ...
    if map_mode or digest:
        ...
        if digest:
            ...
            for item in institutions:
                try:
                    results.append({key: value for key, value in desk.send_digest(item, only_if_due=True).items() if key != "incidents"})
