...
The assistant page holds a two-way spoken conversation, started with the Voice button in the composer. The microphone stays on while Agentic Saffron speaks, so the person can talk over a reply: it stops at once and the new question is answered. Saying "stop", "ruko", "bas" or "ನಿಲ್ಲಿಸು" only silences it. Phrases said with a short pause become one request, and the echo of Agentic Saffron's own voice is ignored. On iPhone and iPad, where the microphone and speaker cannot both run, it listens between replies and shows a Stop button.
...
- **Indian voice and languages.** Replies come back in the language spoken: Indian English, Hindi (Devanagari or Hinglish) or Kannada. With `SAFFRON_VOICE_TTS_PROVIDER=polly` they are spoken by Amazon Polly's Kajal (neural, en-IN and hi-IN), sentence by sentence. Otherwise, and for Kannada, the device's own Indian voice speaks.
