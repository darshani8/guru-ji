...
- Bounded browser voice transport with a ten-session cap, five-minute expiry, principal/scope-bound one-use WebSocket tickets, origin checks, binary-frame rejection, per-session utterance limits, and ephemeral audio-retention semantics. The browser requests microphone permission, uses Web Speech recognition/TTS, sends only final transcripts, and routes them through the same policy-bound assistant service as text chat.
...
- Briefing execution and briefing history now use separate explicit capabilities and College scope filtering.
- A local Cerbos-shaped, deny-by-default policy decision point protects semantic connector execution with versioned policy and decision IDs; a future Cerbos sidecar can replace the implementation behind the same boundary.
- Remote connector requests carry only server-derived principal and effective College/Department/Batch scope, bounded semantic tool names, request IDs, and query limits; arbitrary SQL is never exposed to the model or connector contract.
- Versioned closed Pydantic SSE events and discriminated Pydantic WebSocket inbound events protect chat and voice transport; the browser-compatible answer/done event behavior is retained.
- Provider capabilities and normalized model events are explicit for deterministic and Ollama providers; provider-specific response objects remain behind the adapter.
- Redaction-safe backend-neutral trace spans and structured policy/provider audit metadata are recorded without prompts, learner data, credentials, SQL rows, raw tool results, or audio. Audit events are append-only.
...
- Docker dependency installation is aligned with runtime imports, and the checked-in OpenAPI file is generated from the live route surface with parity validation.
...
The exported branch snapshot was checked with Python 3.12 and the declared runtime dependencies installed. Ninety-five automated tests pass, including HTTP contract, JSON/SSE chat, typed voice transport, persistence, OIDC/JWT verification, deny-by-default PDP behavior, briefing capability/scope checks, model-provider fallback, remote connector identity/scope contracts, redaction-safe tracing, public-web provider/extraction/allowlist boundaries, explicit web-chat routing, configuration-hardening, request-size, timeout, rate-limit, security-header, and stable-error tests. Python compilation passes. Live-route OpenAPI parity validation, repository hygiene, undefined-name linting, and formatting gates pass. The live Uvicorn smoke test still needs to be rerun after this change set. Public-web transport was validated with deterministic HTTP mocks; no live search-provider credential was used in this validation run. PostgreSQL adapter construction and configuration paths are covered, but no live PostgreSQL server was available in this validation run.

## Intentionally not claimed

This is a hardened local vertical slice, not a production deployment. It does not include a live institutional database, the institution's real connector service, the institution's real OIDC issuer/JWKS configuration, a production Ollama/model deployment, hosted-model credentials, raw-audio WebRTC signaling, provider-backed STT/TTS, secret management, or production observability. The College A connector remains deterministic demo data unless a remote connector is explicitly configured. The browser voice MVP receives final transcripts only; browser speech providers and their privacy terms remain outside Guru Ji's control. Those integrations need the actual institution contracts, security approvals, and deployment secrets rather than invented placeholders. PostgreSQL, OIDC/JWT verification, the remote connector contract, the optional local model boundary, and the browser transcript transport now exist, but they still need private services, credentials, connection-pool tuning, identity-claim mapping approval, model evaluation, and deployment testing before production use.
