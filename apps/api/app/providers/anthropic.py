...
The SDK is an optional extra (``anthropic``); importing this module does not
require it, so deterministic, Ollama and LiteLLM deployments are unaffected.
Provider failures surface as a generic public error so callers fall back to
their deterministic path; the specific reason is logged for the operator.
"""
...
def _describe_failure(exc: Exception) -> str:
    ...
    if isinstance(exc, anthropic.NotFoundError):
        return "the model ID is not available to this API key"
...
class AnthropicProvider:
    ...
    def _messages(self) -> Any:
        if self.client is None:
            ...
            self.client = anthropic.AsyncAnthropic(api_key=self.api_key, timeout=self.timeout_seconds)
