...
from app.institution_data.schema import ADDED_COLUMNS, TENANT_TABLES
...
class _CatalogBackend(SqlBackend):
    ...
    def __init__(self, *, columns_present: bool = True, rls_enforced: bool = True) -> None:
        super().__init__()
        self.columns_present = columns_present
        self.rls_enforced = rls_enforced
    ...
    def fetchall(self, sql: str, params=()) -> list[dict]:
        ...
        if "information_schema.columns" in statement and "data_type = 'real'" in statement:
            return []
        if "information_schema.columns" in statement:
            return [{"present": 1}] if self.columns_present else []
        if "pg_class" in statement:
            return [{"relname": table} for table in TENANT_TABLES] if self.rls_enforced else []
        return []
...
class InstitutionDataStoreStartUpTests(unittest.TestCase):
    def test_a_migrated_database_gets_no_alter_table(self) -> None:
        ...
        self.assertEqual([sql for sql in backend.statements if sql.startswith("ALTER TABLE")], [])
        ...
        self.assertTrue(any("CREATE POLICY" in sql for sql in backend.statements), "the policy check itself takes no table lock")

    def test_missing_columns_and_row_level_security_are_added_in_the_same_transaction(self) -> None:
        backend = _CatalogBackend(columns_present=False, rls_enforced=False)
        InstitutionDataStore(backend=backend)
