...
from app.internet_intelligence.map.grading import EXPIRED_DETAIL, FREE_TEXT_KINDS, expire_quotes, grade, superseded_observations
...
class MapRetentionTests(unittest.TestCase):
    ...
    def test_pruning_one_institution_leaves_another_alone(self):
        ...
        self.assertEqual({row["evidence_id"]: row["detail"] for row in self.store.list_evidence("second", asset_id=second["account"])}[second["old_snippet"]], PERSONAL)
