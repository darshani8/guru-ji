...
class DigestMemoryTests(unittest.TestCase):
    ...
    def test_one_institutions_failure_does_not_hold_back_the_others(self):
        ...
        results = asyncio.run(run_monitor._run(types.SimpleNamespace(platform=platform), None, digest=True))
        self.assertEqual(sent, ["b", "c"])
        self.assertEqual(results[0], {"institution_id": "a", "error": "outbox insert failed"})
        self.assertFalse(asyncio.run(run_monitor._run(types.SimpleNamespace(platform=platform), None, digest=True))[1]["sent"], "the hourly pass does not send b's twice")
